...
def create_annotation_ouput(p: Path):
    ...
    """ Trim zeros triplets """
    # zeros_triplets_ix = np.unique(np.where(points == np.array([[0, 0, 0]]))[0])
    # non_zeros_triplets_ix = np.unique(np.where(points != np.array([[0, 0, 0]]))[0])
    #
    # # sanity check
    # assert len(zeros_triplets_ix) + len(non_zeros_triplets_ix) == points.shape[0]
    #
    # non_zero_points = deepcopy(points[non_zeros_triplets_ix])
    # # points = non_zero_points
    # colors = deepcopy(colors[non_zeros_triplets_ix])
...
if __name__ == "__main__":
    ...
    for ply_file in tqdm(source_dir.glob('*.ply')):
        pcd = create_annotation_ouput(ply_file)
